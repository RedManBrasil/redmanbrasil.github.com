...
function showNumbers(a, b, c) {
    ...
    if( b == 'bitcoin'){ //se a moeda for o BTC exibir USD ao invés de BTC
        $('#' + b + '-total-renata').append( "US$" + (c * document.getElementById ( b +'-amount-renata' ).innerText).toFixed(2));
        }
    else{
        if ( ($('#'+ b +'-amount-renata').html()) ) { //tem que checar se existe a caixinha com a id requirida
            $('#'+ b +'-total-renata').append( (c * document.getElementById ( b +'-amount-renata' ).innerText).toFixed(8) + " BTC");
        }
    }
...
$(document).ajaxComplete(function(event,xhr,settings){
    ...
    { //faz a soma do valor em btc dos portfolios, para o total
        ...
        $('#Rtable .Rbtcvalue').each(function() {
            id_aux = $(this).attr('id').replace("-total-renata", "").replace("-amount-renata", ""); //nome da moeda a qual se pega a informação
            ...
            tot_btc += aux_btc;
            tot_real_R += aux_real;
            $('#' + id_aux + '-real-renata').append('R$' + aux_real.toFixed(2) );
        });
        $('#Rtable .Rbtcvalue').each(function() {
            id_aux = $(this).attr('id').replace("-total-renata", "").replace("-amount-renata", ""); //nome da moeda a qual se pega a informação
            aux_btc = parseFloat($(this).text());
        ...
        $('#total-renata-bitcoin').append('<strong>'+ tot_btc.toFixed(8) +' BTC</strong>');
        $('#total-renata-real').append('<strong>R$'+ tot_real_R.toFixed(2) +'</strong>');
        console.log('Renata:');
