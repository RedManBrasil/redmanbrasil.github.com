		...
		function getPic() {
			setTimeout(
				function () {
			FB.api('/me', 'GET', {fields: 'first_name,last_name,name,id,picture.width(150).height(150)'}, function(response) {
				document.getElementById('resposta').innerHTML = "<img src='" + response.picture.data.url + "' style='border-radius:50%;border:6px solid #0aa046;width:10%; margin-top: 20px;width:590px;max-width:590px;'>";
			});
			} , 600);
		}
		...
		function getName2() {
  			setTimeout(
				function () {
					FB.api('/me', 'GET', {fields: 'first_name,last_name,name,id,picture.width(150).height(150)'}, function(response) {
						document.getElementById('resposta2').innerHTML = "&nbsp;&nbsp;Welcome " + response.first_name;
						});
				} , 600);
  		}
